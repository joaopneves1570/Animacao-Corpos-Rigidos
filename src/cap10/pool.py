...
class Main:
    def __init__(self, qtd_corpos = 5, largura = 800, altura = 600, titulo = "Simulação do estouro de uma partida de bilhar"):
    ...
    def setupCena(self):
        ...
        view = glm.lookAt(glm.vec3(0, 40, 0), glm.vec3(0, 0, 0), glm.vec3(0, 0, -1))
        ...
        for linha_idx, qtd_pinos in enumerate(configuracao_linhas):
            ...
            for pino_idx in range(qtd_pinos):
                ...
                # Instancia mesh das bolas
                mesh_bola = Mesh("objs/esfera.obj", cor=(np.random.uniform(0, 1, 3)))
                # Instancia o corpo físico rígido (usando o modelo do pino)
                body = RigidBody("objs/esfera.obj", pos_inicial, massa=5.0, gravidade=True)
                
                self.fisicaMundo.addBody(body)
                self.entidades.append(Entity(body, mesh_bola))
